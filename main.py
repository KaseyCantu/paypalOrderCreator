...
def send_money():
    ...
    # browser.find_element(By.XPATH, '//button[contains(text(), "Next")]').click()  # Send Money Recipient | Next button
    browser.find_element(By.XPATH, f'//div[contains(text(), "{recipient}")]').click()  # Send Money | Next button


def enter_funding_amount():
    # sleep(2)
...
def send_money_to_create_orders():
    send_money()
